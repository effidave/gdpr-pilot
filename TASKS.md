...
- [ ] Test the `jl4-wasm` in-browser build (the spike tested `jl4-service` over REST but not WASM), since the design assumes facts never leave the provider's machine
      *(from: L4 spike - integration criterion - 2026-09-23)*
- [ ] Raise with Legalese: `l4 batch` wrapper fails when a MAYBE parameter precedes another; a failing `#ASSERT` exits 0; `l4 run` never captures `#EVALTRACE` traces; `@ref` does not reach traces or output; `/query-plan` ignores numeric arguments; status of RUNTIME-INPUT-STATE-SPEC / TYPICALLY
