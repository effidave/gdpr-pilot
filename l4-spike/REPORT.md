...
| 2 | **Relevant questions** | 3 | 4 | L4's planner (`:qp`) does real three-valued partial evaluation and ranks the questions still needed (`results/l4/probes/queryplan.txt`), but: (a) BOOLEAN decisions only, so it can't run on the tri encoding (`queryplan-tri.txt`); (b) numbers don't reach it: the REPL rejects `users=2500000`, and the service's `/query-plan` endpoint accepts `monthly active UK users: 2500000` but still returns `determined: null` and asks for that same input again (`results/l4/service/4b.json`), so it can't settle the S4 case; (c) the REPL can't address backtick names with spaces. Baseline lists the missing facts that could change each result (e.g. S2-U: `group_staff_headcount`), but doesn't rank them. |
...
| 8 | **Integration** | 3 | 4 | L4: the `jl4-service` REST service works. The spike bundle deployed and evaluated, a missing plain input returned an explicit error, and MAYBE nulls became NOTHING (`results/l4/service/`). OpenAPI/MCP/JSON batch also exist; the WASM build was not tested. Enum results come back with backticks inside the strings (e.g. "`Threshold not met`"). Problems found: the `l4 batch` wrapper fails to parse when a MAYBE parameter comes before another (`rules/probes/batchbug.l4`); records come back as positional arrays without field names; a row that fails to decode reports `"status":"success"`; and it takes about 10 seconds per row (70 rows in 12 minutes). Baseline: plain TypeScript, runs in a browser or Node, and facts never leave the machine. There's no schema or REST out of the box. |
| 9 | **Tests** | 3 | 4 | L4: `#ASSERT` in the language is pleasant (`rules/scenarios.l4`, 17 assertions pass), but **a failing `#ASSERT` still exits 0**, so CI needs output parsing. `rules/run-l4.mjs --check` wraps batch as a regression runner (70 expectations pass; about 12 minutes per run). Baseline: `npm test`, 130 expectations, exits 1 on failure (checked with a planted failure). |
...
| `jl4-service` | Built last. Deployed the spike bundle and tested it over REST (`results/l4/service/`). The batch MAYBE bug does not affect the service. | None needed. Over REST, idiomatic probe E still returns `false` for unknown users, and `/query-plan` ignores numeric arguments. |
