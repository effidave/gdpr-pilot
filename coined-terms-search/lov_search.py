...
Notes
- Written against the LOV API documentation; not yet tested against live
  responses. If a field comes out blank, check a file in <out>/raw/ and adjust
  parse_result() or vocab_details(). Nothing is lost, as raw JSON is kept.
- The docs spell the type filter "propery". The script sends "property"; if
  that returns nothing it retries unfiltered and filters locally.
...
BASE = "https://lov.linkeddata.es/dataset/lov/api/v2"
...
def parse_result(r):
    return {
        ...
        "vocab": first(r.get("vocabulary.prefix")),
        "score": float(r.get("score") or 0),
        "occurrences": first(r.get("metrics.occurrencesInDatasets")),
        "reused_by_datasets": first(r.get("metrics.reusedByDatasets")),
